...
class Chapter6IndexViewTests(TestCase):
    ...
    def test_index_pages(self):
        ...
        page_li_entries_regex = {
            ...
            'Official Django Tutorial': r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)("https://docs.djangoproject.com/en/2.0/intro/tutorial01/"|\'https://docs.djangoproject.com/en/2.0/intro/tutorial01/\')(\s*)>(\s*|\n*)Official Django Tutorial(\s*|\n*)</a>(\s*|\n*)</li>',
        ...
        for expected_regex in expected_pages_li:
            self.assertTrue(re.search(expected_regex, self.content), f"{FAILURE_HEADER}Checks for the top five pages in the index() view's response failed. Check you are using the correct list of objects, the correct HTML markup, and try again.{FAILURE_FOOTER}")
...
class Chapter6NoItemsIndexViewTests(TestCase):
    ...
    def test_sample_category(self):
        ...
        updated_response = str(self.client.get(reverse('rango:index')).content)
...
class Chapter6BadCategoryViewTests(TestCase):
    ...
    def test_nonexistent_category(self):
        ...
        self.assertIn(lookup_string, str(response.content), r"{FAILURE_HEADER}The expected message when attempting to access a non-existent category was not found. Check your category.html template.{FAILURE_FOOTER}")
    ...
    def test_empty_category(self):
        ...
        self.assertIn(lookup_string, str(response.content), r"{FAILURE_HEADER}The expected message when accessing a category without pages was not found. Check your category.html template.{FAILURE_FOOTER}")